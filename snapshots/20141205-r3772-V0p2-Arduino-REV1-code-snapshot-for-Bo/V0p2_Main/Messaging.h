...
// From: https://github.com/bblanchon/ArduinoJson if used...
...
//    bool put(SimpleStatsKey key, int newValue, GenericStatsDescriptor *descriptor = NULL);
  ...
  protected:
    ...
    //    // Type-safe union of legit JSON values.
    //    ArduinoJson::Generator::JsonValue value;
...
//        bool thisRun : 1;
...
#if defined(ALLOW_JSON_OUTPUT)
    // Write '\0'-terminated stats string to provided buffer; returns a non-zero character count if successful.
    // The returned character count does not include the trailing null.
    // At most bufSize-2 characters are written before the null
    // so that overrun/truncation can be easily detected.
    uint8_t _writeJSON(ArduinoJson::Generator::JsonObjectBase &job,
                       uint8_t *buf, uint8_t bufSize, uint8_t sensitivity, bool maximise, bool suppressClearChanged);
#endif
  ...
  private:
    ...
    // Small write counter (and flag to enable it).
    // Helps to track lost transmissions of generated stats.
    // Count field increments after a successful write,
    // and wraps back to zero after 63 (to limit space on the wire);
    ...
    struct WriteCount
      ...
      uint8_t count : 6; // Increments on each successful write.
      } c;
  };
