...
uint8_t getFROSTTargetC()
  ...
  // TODO-403: cannot use hasEcoBias() with RH% as tha would cause infinite recursion!
...
// Returns true iff there is a full set of stats (none unset) and this 3/4s of the values are lower than supplied sample.
// Always returns false if all samples are the same.
//   * s is start of (24) sample set in EEPROM
//   * sample to be tested for being in lower quartile
...
//  if((STATS_UNSET_INT != smoothedAmbLight) &&
//     ((0 == smoothedAmbLight) || inBottomQuartile((uint8_t *)EE_STATS_START_ADDR(EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED), smoothedAmbLight)))
//    { return(false); }
#endif
...
//  const uint8_t smoothedOccPC = eeprom_read_byte((uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_OCCPC_BY_HOUR_SMOOTHED) + hh));
//  if((STATS_UNSET_INT != smoothedOccPC) &&
//     ((0 == smoothedOccPC) || inBottomQuartile((uint8_t *)EE_STATS_START_ADDR(EE_STATS_SET_OCCPC_BY_HOUR_SMOOTHED), smoothedOccPC)))
//    { return(false); }
#endif
...
void ModelledRadValve::computeTargetTemperature()
  ...
  if(!inWarmMode()) // In FROST mode.
    ...
    // If scheduled WARM is due soon then ensure that room is at least at full setback temperature
    ...
    if((isLikelyUnoccupied() || AmbLight.isRoomDark() /* || inOutlierQuartile(false, EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED) */ ) &&
       !isAnyScheduleOnWARMNow()) // TODO-111: suppress set-back from lack of occupancy in scheduled on-time...
      {
...
//      const uint8_t lingerThreshold = DEFAULT_MIN_VALVE_PC_REALLY_OPEN - 1; // Should be below 'call for heat' threshold.
    ...
    const uint8_t minReallyOpen = NominalRadValve.getMinPercentOpen();
    const uint8_t targetPO = fnmax(targetPORaw, minReallyOpen);
#else
    // Use as-is.
...
#if !defined(ENABLE_BOILER_HUB) && defined(DEBUG)
...
#if 0 && defined(DEBUG) && defined(ENABLE_VOICE_SENSOR)
  DEBUG_SERIAL_PRINT_FLASHSTRING("Voice level=");
  DEBUG_SERIAL_PRINT(Voice.get());
  DEBUG_SERIAL_PRINT_FLASHSTRING(" detected=");
  DEBUG_SERIAL_PRINT(Voice.isVoiceDelected());
  DEBUG_SERIAL_PRINTLN();
#endif
