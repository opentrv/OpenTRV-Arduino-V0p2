...
// Setback period before WARM period to ensure that WARM target can be reached on time.
// Important for slow-to-heat rooms that have become very cold.
// Not much longer than PREWARM_MINS so that we can safely use this any time we could use PREWARM_MINS.
#define PREPREWARM_MINS ((3*LEARNED_ON_PERIOD_M) >> 1)
