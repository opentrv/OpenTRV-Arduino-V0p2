...
// Sensor for ambient light level; 0 is dark, 1023 is bright.
class AmbientLight : public Sensor<uint16_t>
  {
  private:
    // Ambient light value [0,1023] dark--light.
    uint16_t value;
...
#ifdef DIRECT_MOTOR_DRIVE_1
// Actuator for direct local (radiator) value motor control.
class ValveMotorDirect : public AbstractRadValve
  {
  public:
    virtual uint8_t read();
  };
extern ValveMotorDirect ValveDirect;
#endif
