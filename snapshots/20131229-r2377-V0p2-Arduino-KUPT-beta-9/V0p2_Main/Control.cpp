...
#endif
// Set maximum valve slew rate when close to target temperture.
...
#define TRV_MAX_SLEW_PC_PER_MIN 5 // Maximum normal slew rate, eg to fully open from off when well under target; [1,100].
#else
...
static bool computeRequiredTRVPercentOpen()
  ...
  if(currentTempC < targetTempC) // (Well) under temp target: open valve.
    ...
    const uint8_t targetPO = tmp * ulpStep; // Now in range 6 to 96, eg valve nearly shut just below top of 'correct' temperature window.
