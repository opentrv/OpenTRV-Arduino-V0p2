...
uint8_t getFROSTTargetC()
  ...
  const uint8_t result = (!hasEcoBias() || (isRHAvailable() && isRHHigh())) ? BIASCOM_FROST : BIASECO_FROST;
...
uint8_t getFROSTTargetC()
  ...
  // Prevent falling to lowest frost temperature if relative humidity is high (eg to avoid mould).
  if(!hasEcoBias() || (isRHAvailable() && isRHHigh())) { return(fnmax(stored, BIASCOM_FROST)); }
...
static volatile uint8_t occupationCountdownM;
...
uint8_t occupancyConfidencePercent()
  ...
  return(fnmin(100 - ((OCCUPATION_TIMEOUT_M - occupationCountdownM) << OCCCP_SHIFT), 100));
  }
...
bool shouldBeWarmedAtHour(const uint_least8_t hh)
  ...
  // Thus avoid any 'smart' warming for at least 25% of the daily cycle. EE_STATS_SET_AMBLIGHT_BY_HOUR
  const uint8_t smoothedAmbLight = eeprom_read_byte((uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED) + hh));
  if((STATS_UNSET_INT != smoothedAmbLight) && inBottomQuartile((uint8_t *)EE_STATS_START_ADDR(EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED), smoothedAmbLight))
    { return(false); }
#endif
...
static volatile bool callingForHeat;
...
void computeTargetTemperature()
  ...
  if(!inWarmMode()) // In FROST mode.
    ...
    if(((!scheduledOn) || hasEcoBias()) && // TODO-111: suppress set-back from lack of occupancy in scheduled on-time in comfort mode.
      ...
      // Use a bigger setback if eco biased, unless in a scheduled on-time.
      const uint8_t setback = ((!scheduledOn) && hasEcoBias()) ? SETBACK_ECO : SETBACK_DEFAULT;
#ifdef ENABLE_ANTICIPATION
...
static bool computeRequiredTRVPercentOpen()
  ...
  if(currentTempC < targetTempC) // (Well) under temp target: open valve.
    ...
    if(TRVPercentOpen != 100)
      ...
      // Ensure open quickly from cold unless glacial...
      if(0 == TRVPercentOpen) { TRVPercentOpen = fnmax((uint8_t)TRV_SLEW_PC_PER_MIN_VFAST, getMinValvePcReallyOpen()); }
    ...
    const uint8_t targetPO = fnmax(targetPORaw, getMinValvePcReallyOpen());
#else
    // Use as-is.
...
#if !defined(TRV_SLEW_GLACIAL)
          const uint8_t maxSlew = (inBakeMode() || !hasEcoBias() || (TRVPercentOpen < getMinValvePcReallyOpen())) ?
...
//  else { eeprom_smart_update_byte(phT, smoothStatsValue(tempSmoothed, temp)); }
  ...
  simpleUpdateStatsPair(EE_STATS_SET_AMBLIGHT_BY_HOUR, hh, ambL);
//  // Update the last-sample slot using the mean samples value.
//  eeprom_smart_update_byte((uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_AMBLIGHT_BY_HOUR) + hh), ambL);
//  // If existing smoothed value unset or invalid, use new one as is, else fold in.
//  uint8_t *const phA = (uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED) + hh);
...
void setupOpenTRV()
  ...
  ATOMIC_BLOCK (ATOMIC_RESTORESTATE)
    ...
    PCMSK2 = 0b00000001; // PD; PCINT 16--24 (0b1 is PCINT16/RX)
    }
...
ISR(PCINT2_vect)
  ...
  // Ensure that the CLI is active, eg from RX activity.
  // It is OK to tigger this from other things such as button presses.
  resetCLIActiveTimer();
  }
#endif
...
//  if(useExtraFHT8VTXSlots) { DEBUG_SERIAL_PRINTLN_FLASHSTRING("ES@0"); }
  ...
  if(recompute || recentUIControlUse())
    {
    computeTargetTemperature(); // Force immrecompute of temperature for (UI) responsiveness.
    }
...
//  DEBUG_SERIAL_PRINTLN();
  ...
  switch(TIME_LSD) // With TWO_S_TICK_RTC_SUPPORT only even seconds are available.
    ...
    case 2: { if(runAll) { seedRNG8(minuteCount ^ cycleCountCPU() ^ (uint8_t)Supply_mV.get(), getSubCycleTime() ^ (uint8_t)getAmbientLight(), (uint8_t)getTemperatureC16()); } break; }
