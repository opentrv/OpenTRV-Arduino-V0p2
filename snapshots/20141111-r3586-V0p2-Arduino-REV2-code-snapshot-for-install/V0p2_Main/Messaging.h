...
// Returns true unless the buffer clearly does not contains a simple raw JSON message.
// This message is expected to be one object wrapped in '{' and '}'
// and containing only ASCII printable characters in the range [32,126].
...
//int8_t startRawJSONMessage(uint8_t *buf);
//int8_t appendToRawJSONMessage(uint8_t *buf, int8_t pos, const __FlashStringHelper *format, ...);
//int8_t endRawJSONMessage(uint8_t *buf, int8_t pos);
...
// If secure is true then this message arrived over a secure channel.
// The buffer content is not altered.
