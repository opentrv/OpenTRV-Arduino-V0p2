...
//// Return last value returned by read(); undefined before first read().
//// Fast.
//// NOT thread-safe or usable within ISRs (Interrupt Service Routines).
//uint16_t SupplyVoltageMilliVolts::get() { return(getBatterymV()); }

//// Returns true if the sensor reading value passed is potentially valid, eg in-range.
//// Anything computed by by read() is potentially valid.
//bool SupplyVoltageMilliVolts::isValid(const uint16_t value) { return(true); }
//
...
//bool SupplyVoltageMilliVolts::isSupplyVoltageLow() { return(isBatteryLow()); }
