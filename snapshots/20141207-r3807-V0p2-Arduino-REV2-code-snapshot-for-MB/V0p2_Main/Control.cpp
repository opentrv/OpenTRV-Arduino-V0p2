...
void ModelledRadValve::computeTargetTemperature()
  ...
  if(!inWarmMode()) // In FROST mode.
    ...
    if((Occupancy.isLikelyUnoccupied() || AmbLight.isRoomDark() /* || inOutlierQuartile(false, EE_STATS_SET_AMBLIGHT_BY_HOUR_SMOOTHED) */ ) &&
...
//      const uint8_t lingerThreshold = DEFAULT_MIN_VALVE_PC_REALLY_OPEN - 1; // Should be below 'call for heat' threshold.
    ...
    if(targetPO != value)
      ...
      const bool minimiseSlew = AmbLight.isRoomDark() || Occupancy.isLikelyUnoccupied();
...
//   * doBoth  send binary form alternately with JSON form if both are supported
...
DEBUG_SERIAL_PRINTLN_FLASHSTRING("Bin gen err!");
    ...
    ss1.put(Occupancy.twoBitTag(), Occupancy.twoBitOccupancyValue()); // Reduce spurious transmission.
    ss1.put("tTarg|C", NominalRadValve.getTargetTempC());
...
//  DEBUG_SERIAL_PRINTLN();
  ...
  switch(TIME_LSD) // With TWO_S_TICK_RTC_SUPPORT only even seconds are available.
    ...
    case 56:
      {
#ifdef OCCUPANCY_SUPPORT
      // Update occupancy status (fresh for target recomputation below).
      Occupancy.read();
#endif
