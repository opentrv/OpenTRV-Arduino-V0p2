...
class RoomTemperatureC16 : public Sensor<int>
  ...
  private:
    ...
    // Previous values, 0 being the newest, and following ones successively older.
    // Half the filter size times the read() interval gives the approximate time constant.
    // Note that full response time of a typical mechanical TRV is ~20mins.
    // Always at least 4 elements, and should be a power of 2.
    int prev[8];

    // True if median filtering is on and read() should be called regularly.
    bool filteringOn;
  ...
  public:
    ...
    // The the raw unfiltered value last read from the sensor.
    int getRaw() const { return(prev[0]); }

    // Get recent maximum raw value.
    int getRecentMaxRaw() const;

    // Get recent minimum raw value.
    int getRecentMinRaw() const;

    // Returns true if filtering is on, eg because recent readings have been jittery or fast changing.
    // This may be caused by eg draughts or turbulent air or direct radiation from a radiator.
    // When true read() should be called at regular intervals.
    // When true the values from this sensor will change more slowly that otherwise
    // and control loops dependent on this sensor may wish to be less aggressive to avoid overshoot.
    bool isFilteringOn() const { return(filteringOn); }

    // Returns true if the underlying sensor precision is coarser than 1/16thC.
...
class ValveMotorDirectV1 : public AbstractRadValve
  ...
  private:
    ...
    // Motor drive status.
    // Marked volatile for thread-safe (simple) lock-free access.
    volatile motor_drive motorDriveStatus;
...
//    virtual bool handleInterruptSimple();
    ...
    // Get current motor drive status (off or direction of running).
    // Thread-/ISR- safe.
    motor_drive getMotorDriveStatus() const { return(motorDriveStatus); }
  };
// Singleton implementation/instance.
