...
// Minutes until locally-controlled boiler should be turned off; boiler should be on while this is positive.
...
//  const bool sensorReading30s = (TIME_LSD >= 30);
  ...
  if(hubMode)
    ...
    if((boilerCountdownM <= 2) && // Don't lose a final TX that would keep the boiler on...
       (boilerCountdownM != 0)) // But don't force unit to listen/RX all the time if no recent call for heat.
      { needsToEavesdrop = true; }
    else if((!heardIt) &&
       (!minute0From4ForSensors) &&
       (boilerCountdownM <= 4)) // Listen eagerly for fresh calls for heat for last few minutes before turning boiler off.
      {
