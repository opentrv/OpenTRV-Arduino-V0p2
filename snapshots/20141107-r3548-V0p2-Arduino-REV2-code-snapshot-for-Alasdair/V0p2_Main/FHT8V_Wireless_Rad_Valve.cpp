    ...
    for(pos = 0; pos < 6; ++pos)
      ...
      if(0xcc != b)
        ...
        if(MSG_JSON_LEADING_CHAR == b)
          {
          // Might be completely mangled...
          // Force a terminating '\0' to prevent overrun.
          FHT8VRXHubArea[sizeof(FHT8VRXHubArea)-1] = '\0';
#if 0 && defined(DEBUG)
          DEBUG_SERIAL_PRINT_FLASHSTRING("Possible JSON msg: ");
          DEBUG_SERIAL_PRINT((const char *)(FHT8VRXHubArea + pos));
          DEBUG_SERIAL_PRINTLN();
#endif
          // Check for terminating '}' then CRC; reject if not found.
          // Note that JSON nominally allows nested structures, and thus multiple '}'s.
          for(int i = pos + 1; i < sizeof(FHT8VRXHubArea); ++i)
            {
            const char c = FHT8VRXHubArea[i];
            // Reject any non-printable-ASCII characters; may be transmission noise.
            if((c < 32) || (c > 126)) { break; } // Give up.
            // Look for putative end of (outer) object.
            if('}' == c)
              {
              // Look for following CRC and check that it is valid.
              if('\0' == FHT8VRXHubArea[i+1]) // FIXME: is '\0' for now...
                {
                FHT8VRXHubArea[i+1] = '\0'; // Terminate JSON string...
