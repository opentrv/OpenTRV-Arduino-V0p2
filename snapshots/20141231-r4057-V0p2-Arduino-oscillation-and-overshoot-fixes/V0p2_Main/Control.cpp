  ...
  if(!inWarmMode())
    ...
    if(longVacant ||
      ((
        ...
        AmbLight.isRoomDark() ||
#else
        // TODO // AmbLight.isRoomLongDark() || // Been dark for a while now...
#endif
        notLikelyOccupiedSoon) && !isAnyScheduleOnWARMNow()))
      {
...
//
////  // True if latest raw temperature nominally at/above top of target range.
////  const int rawTargetC16 = ((int)mrv.getTargetTempC())<<4;
////  if(TemperatureC16.getRaw() >= rawTargetC16 + refTempOffsetC16) { return(true); }
////  // True if the most recent change in temperature is upwards by a big fraction of the 1C proportional range.
////  // This is implies that temperature may be rising too fast to control easily and may overshoot.
////  if(TemperatureC16.getRawDelta() >= 0x6) { return(true); } // ~.4C between measurements is too much.
...
////  // True if temperature range over recent history is much wider than the 1C proportional control range.
////  const int minRecent = TemperatureC16.getRecentMinRaw();
////  if(maxRecent - minRecent >= (0x18)) { return(true); } // 1.5C is too much!
//
//  return(false);
//  }
...
//      const uint8_t lingerThreshold = DEFAULT_MIN_VALVE_PC_REALLY_OPEN - 1; // Should be below 'call for heat' threshold.
    ...
    // Ensure that the valve is fully shut.
    return(0);
    }
...
//  // (high enough that it might not be possible to turn the heat down quickly enough to avoid a significant overshoot)
...
//DEBUG_SERIAL_PRINTLN_FLASHSTRING("slightly under temp");
    ...
    // Don't open valve up further in soft setback if within top half of proportional range.
    // This should effectively provide a low-effort ~0.5C setback.
    ...
    // Open glacially if explicitly requested or if temperature overshoot has happened or is a danger.
    // Also be glacial if in soft setback which aims to allow temperatures to drift gently down a little.
    //   (TODO-451 - Have darkness only immediately trigger a 'soft setback')
    // Be glacial if the raw temperature rose on the last tick: this is aiming for steady temperatures.
    // This assumes that most valves more than about 1/3rd open can deliver significant power, esp if not statically balanced.
    const bool beGlacial = inputState.glacial || inputState.softSetback ||
...
#if defined(QUICK_DARK_SETBACK_IS_SOFT)
  // Allow gentle small extra setback when the room is dark and no schedule is active (TODO-111).
  inputState.softSetback = AmbLight.isRoomDark() && !isAnyScheduleOnWARMNow();
#endif
