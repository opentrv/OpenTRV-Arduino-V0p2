...
// Enter standby mode.
// SPI must already be configured and running.
static void _RFM22ModeStandby() { _RFM22WriteReg8Bit(RFM22REG_OP_CTRL1, 0); }

// Enter transmit mode (and send any packet queued up in the TX FIFO).
// SPI must already be configured and running.
static void _RFM22ModeTX() { _RFM22WriteReg8Bit(RFM22REG_OP_CTRL1, 9); } // TXON | XTON

// Enter receive mode.
// SPI must already be configured and running.
static void _RFM22ModeRX() { _RFM22WriteReg8Bit(RFM22REG_OP_CTRL1, 5); } // RXON | XTON
...
void RFM22RawStatsTX(const bool isBinary, uint8_t * const buf, const bool resumeRX, const bool doubleTX)
  {
