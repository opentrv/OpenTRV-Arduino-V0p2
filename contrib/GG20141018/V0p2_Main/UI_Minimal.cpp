...
static void handleLEARN(const uint8_t which)
  ...
  if(isWarmModeDebounced) { setSimpleSchedule(RTC::getMinutesSinceMidnightLT(), which); }
...
//  const bool rUIcu = recentUIControlUse();
  ...
  if(fastDigitalRead(BUTTON_MODE_L) == LOW)
    ...
    if(isWarmModeDebounced || statusChange) // Generate flash(es) if in WARM mode or fiddling with UI other than Mode button.
      ...
      if(!Light::isRoomDark() || statusChange || (forthTick && (0 != Light::readAmbientLight()) && !Light::isRoomDark()))
        {
//#ifdef LEARN_BUTTON_AVAILABLE
...
//#endif
    ...
    // DHD20131223: do not flash if the room is dark so as to save energy and avoid disturbing sleep, etc.
    else if(forthTick && !Light::isRoomDark() && isControlledValveOpen() && (getTRVPercentOpen() >= getMinValvePcReallyOpen()))
      {
...
void checkUserSchedule()
  ...
  const uint_least16_t msm = RTC::getMinutesSinceMidnightLT();
...
void serialStatusReport()
  ...
  const tempC16_t temp = Temp::getTemperatureC16();
  ...
  // *L* ambient light section; range [0,1203].
  Serial.print(F(";L"));
  Serial.print(Light::getAmbientLight());
#endif
...
#ifdef DATE
  printCLILine(deadline, F("A D"), F("set days D"));
#endif
...
static uint8_t sn;
// Utility routine to decode a single arg command
bool isNumArg(uint16_t * d)
  {
  char *last; // Used by strtok_r().
  char *tok1;
  // Minimum 3 character sequence makes sense and is safe to tokenise, eg "C 0".
  const bool is = (sn >= 3) && (NULL != (tok1 = strtok_r(buf+2, " ", &last)));
  if(is)
    {
    *d = (uint16_t) atoi(tok1);
    }
  return(is);
  }
    ...
    switch(buf[0])
      ...
      case 'A':
        ...
        if(isNumArg(&d))
          ...
          {
          Serial.print(RTC::getDaysLT());
          Serial_print_space();
          Serial.print(RTC::getY12());
          }
        Serial.println();
        break;
        }
      ...
      case 'D':
        ...
        if(isNumArg(&setN))
          {
          const uint8_t thisHH = RTC::getHoursLT();
...
//#endif
      ...
      case 'S':
        ...
        Serial.println();
        uint_least8_t hh = RTC::getHoursLT();
      ...
      case 'T':
        ...
        if((n >= 5) && (NULL != (tok1 = strtok_r(buf+2, " ", &last))))
          ...
          if(NULL != tok2)
            ...
            if(!RTC::setHoursMinutesLT(hh, mm)) { InvalidIgnored(); }
            }
          }
        break;
        }
