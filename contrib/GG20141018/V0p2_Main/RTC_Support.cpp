...
// NOT FOR DIRECT ACCESS OUTSIDE RTC ROUTINES.
volatile uint_fast8_t RTC::_secondsLT;
...
void RTC::persistRTC()
  ...
  ATOMIC_BLOCK (ATOMIC_RESTORESTATE)
    ...
    if(days != daysSince1999LT) { eeprom_write_word((uint16_t*)EE_START_RTC_DAY_PERSIST, daysSince1999LT); }
  }
...
const bool RTC::restoreRTC()
  {
  // Restore the persisted days, though ignore if apparently unset (all 1s).
  // FIXME: not sure days persistance logic hangs together.
  const uint16_t days = eeprom_read_word((uint16_t*)EE_START_RTC_DAY_PERSIST);
  if(days != (uint16_t)~0U) { daysSince1999LT = days; }
...
const uint_least16_t RTC::getMinutesSinceMidnightLT() /* const */
  {
...
const uint_least8_t RTC::getMinutesLT() /* const */ {return(getMinutesSinceMidnightLT() % 60);}


// Get local time hours from RTC [0,23].
const uint_least8_t RTC::getHoursLT() /* const */ {return(getMinutesSinceMidnightLT() / 60);}
...
const bool RTC::setHoursMinutesLT(const int hours, const int minutes)
  {
...
void RTC::calcDateLT(const uint_least16_t days)
  {
  // see: http://math.stackexchange.com/questions/536248/how-to-invert-number-of-days-since-1-jan-2000

  // Simplification of the every 4, not 100, 400 leap year rule.
  // With a base date of 2000 (a leap year) all years prior to 2100 follow the simpler 4 year rule
  // meaning it will be out by a day post 2100.

  // Remap day to a quad year.
  const uint16_t qd = days % 1461;  // days in a quad year

  // Remap day to a year.
  const uint16_t yd = (qd < 366) ? qd : ((qd - 1) % 365);  // days in a year

  // Roughly which "month".
  const uint8_t y12t = yd / 30;
  y12 = min((uint8_t) 11, y12t);
  }
...
// Returns true if all OK and the time has been set.
// Thread/interrupt safe, but do not call this from an ISR.
// Will persist days to survive reset as neceessary.
const bool RTC::setDaysLT(const uint_least16_t days)
  {
    if(days != daysSince1999LT);
    {
      daysSince1999LT = days;
      // persistRTC(); FIXME NOT YET
      calcDateLT(days);
    }
...
void RTC::tickMinuteRTC()
  {
    static bool notMidnight;
    if(0 == getMinutesSinceMidnightLT())
    {
      if(notMidnight)
      {
        ++daysSince1999LT;
