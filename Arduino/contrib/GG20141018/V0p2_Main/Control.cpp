...
uint8_t getWARMTargetC()
  ...
  if(potLast != pot || 0 == offsetLast)
    ...
    const uint8_t high = (BIASCOM_WARM+1);
    const uint8_t range = high - low + 1;
    offsetLast = computeWARMTargetC(pot) - (low + (range / 2));
    }

  // TEST: nominal target temp central value covering 12 "months".
  // Winter temp low peaking Apr/Oct dropping away over summer i.e. handling unusually cold summer days
  // Loose approximation of nicol graphs.
  // FIXME: Think RAM is in use
#if 0
  prog_uint8_t monthTarget[] = {19, 20, 21, 21, 20, 19,
                                 18, 19, 20, 21, 20, 19};
  const uint8_t result = (uint8_t) (pgm_read_byte(&(monthTarget[RTC::getY12()]))) + offsetLast;
#else
  const uint8_t monthTarget[] = {19, 20, 21, 21, 20, 19,
                                 18, 19, 20, 21, 20, 19};
  const uint8_t result = monthTarget[RTC::getY12()] + offsetLast;
#endif
  return (result);
  }
#endif
...
uint8_t getWARMTargetC()
  ...
  // TEST: nominal target temp central value covering 12 "months".
  // Winter temp low peaking Apr/Oct dropping away over summer i.e. handling unusually cold summer days
  const /* PROGMEM */ prog_uint8_t monthTarget[] PROGMEM = {19, 20, 21, 21, 20, 19,
                                 18, 19, 20, 21, 20, 19};
  // FIXME: Think RAM is in use
  return (pgm_read_byte(&(monthTarget[RTC::getY12()])));
  }
#endif

#elif defined(TEMP_POT_AVAILABLE)
// Uses cache to avoid expensive recomputation.
// NOT safe in face of interrupts.
uint8_t getWARMTargetC()
  {
...
// Call when some/weak evidence of room unoccupation, such as light going off.
// In this implementation reduces the timeout by half the usual time.
// Do not call from an ISR.
void markAsPossiblyUnoccupied()
  { occupationCountdownM = fmax(occupationCountdownM - OCCUPATION_TIMEOUT_M/2, 0); }
...
void computeTargetTemperature()
  ...
  if(!inWarmModeDebounced()) // In FROST mode.
    ...
    if(inSmartMode())
      ...
      const tempC_t warmTarget = getWARMTargetC();
      const tempC_t currentTempC = Temp::getTemperatureC();
      if(currentTempC < warmTarget) // Only bother with potentially-expensive computations below if not already achieving WARM target!
        ...
        const tempC_t preWarmTempC = fnmax(warmTarget - SETBACK_FULL, frostC); // Putative pre-warm temperature...
        ...
        const uint_least8_t hh = RTC::getHoursLT();
        if(shouldBeWarmedAtHour(hh))
          { targetTempC = (Light::isRoomDark() && isLikelyUnoccupied()) ? preWarmTempC : warmTarget; }
    ...
    if(((!scheduledOn) || hasEcoBias()) && // TODO-111: suppress set-back from lack of occupancy in scheduled on-time in comfort mode.
       Light::isRoomDark() && isLikelyUnoccupied()) // Only setback if apparently unoccupied.
      {
...
static bool computeRequiredTRVPercentOpen()
  ...
  const int8_t tempRelation = Temp::temperatureRelation(targetTempC);
  if(tempRelation < 0) // (Well) under temp target: open valve.
    {
  ...
  else if(tempRelation > 0) // (Well) over temp target: close valve if not yet closed.
    {
    ...
    {
    // Use currentTempC16 16ths to set valve percentage for proportional feedback
    // to provide more efficient and quieter TRV drive and probably more stable room temperature.
    const uint8_t tmp = 16 - Temp::temperatureRelation16ths(targetTempC); // Now in range 1 (at warmest end of 'correct' temperature) to 16 (coolest).
...
void sampleStats(const bool fullSample)
  {
  // TODO: Restructure to delegate responsibilities closer to raw data source and encapsulate?

  // (Sub-)sample processing.

  // Ambient light.
  static lightLevel_t ambLightTotal;
  ambLightTotal += Light::getAmbientLight();

  // Temperature.
  const tempC16_t tempC16 = Temp::getTemperatureC16();
  static tempC16_t tempC16Total;
  tempC16Total += tempC16;

  // WARM mode count.
  static int8_t warmCount;
  warmCount += inWarmModeDebounced() ? 1 : -1;
  ...
  if(fullSample)
    {
    // Average, scale and constrain light to valid range for stats.
    const uint8_t ambL = Light::toStat(ambLightTotal, sampleCount);
    ambLightTotal = 0;

    // Average, scale and constrain temperature to valid range for stats.
    const uint8_t temp = compressTempC16((tempC16Total + (sampleCount/2)) / sampleCount);
    tempC16Total = 0;

    const uint_least8_t hh = RTC::getHoursLT(); // Get the current local-time hour...

    recordStats(hh, ambL, (uint8_t *)EE_START_LAST_AMBLIGHT_BY_HOUR, (uint8_t *)EE_START_LAST_AMBLIGHT_BY_HOUR_SMOOTHED, MAX_STATS_AMBLIGHT);
    recordStats(hh, temp, (uint8_t *)EE_START_LAST_TEMP_BY_HOUR, (uint8_t *)EE_START_LAST_TEMP_BY_HOUR_SMOOTHED, MAX_STATS_TEMP);
    ...
    // Reset accumulators for next sample cycle.
    sampleCount = 0;
...
void populateCoreStats(FullStatsMessageCore_t *const content)
  ...
  content->tempAndPower.tempC16 = Temp::getTemperatureC16();
  content->tempAndPower.powerLow = isBatteryLow();
  content->containsTempAndPower = true;
  content->ambL = fnmax(1, fnmin(254, Light::getAmbientLight() >> 2)); // Coerce to allowed value in range [1,254].
