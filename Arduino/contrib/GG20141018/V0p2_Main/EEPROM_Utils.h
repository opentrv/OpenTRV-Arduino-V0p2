...
#define EE_LEN_SEED 4 // *4-byte* store for part of (non-crypto) random seed.
...
// Generic (4-byte) node ID, of which usually only 2 first bytes are used.
...
// Note that in the presence of (say) an house-code, that takes precedence;
...
#define EE_LEN_ID 4 // Matches a house code in size.  Only first 2 bytes generally used in messages.
...
#define EE_START_STATS_TX_ENABLE 24 // 0xff disables all avoidable stats transmissions; 0 enables all.
// A 1-byte overrun counter, inverted so 0xff means 0.
#define EE_START_OVERRUN_COUNTER 25
