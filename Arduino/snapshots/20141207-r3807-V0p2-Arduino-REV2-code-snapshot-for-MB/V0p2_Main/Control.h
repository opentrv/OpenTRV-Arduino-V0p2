...
// 18C is a safe temperature even for slightly infirm according to NHS England 2014:
...
#define BIASCOM_WARM 23 // Target WARM temperature for Comfort bias; must be in range ]BIASECO_WARM,MAX_TARGET_C-BAKE_UPLIFT-1].
...
// Occupancy measure as a % confidence that room has active occupants.
class OccupancyTracker : public SimpleTSUint8Sensor
  ...
  public:
    ...
    // This sensor prefers to be called once per minute.
    virtual uint8_t preferredPollInterval_s() const { return(60); }
    ...
    // Thread-safe and ISR-safe.
    void markAsOccupied() { occupationCountdownM = OCCUPATION_TIMEOUT_M; value = 100; }
    ...
    // Recommended JSON tag for two-bit value; not NULL.
    ...
    bool increaseCheckForOccupancy() { return(!isLikelyRecentlyOccupied() && isLikelyOccupied()); }
  };
#else
...
class OccupancyTracker
  ...
  public:
    ...
    static uint8_t twoBitOccValue() { return(0); } // Always 0 without OCCPANCY_SUPPORT.
//#define occpancyConfidencePercent() (0) // Always 0 without OCCPANCY_SUPPORT.
  };
#endif
// Singleton implementation for entire node.
