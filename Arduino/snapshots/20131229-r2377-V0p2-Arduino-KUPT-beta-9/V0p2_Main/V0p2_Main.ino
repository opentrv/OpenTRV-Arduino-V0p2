...
#define RX_REDUCE_MIN_M 15 // Minimum minutes quiet before considering reducing RX duty cycle listening for call for heat; [1--255], 10--60 typical.
#define RX_REDUCE_MAX_M 60 // Minimum minutes quiet before considering maximally reducing RX duty cycle; ]RX_REDUCE_MIN_M--255], 60--240 typical.
...
//  const bool sensorReading30s = (TIME_LSD >= 30);
  ...
  if(hubMode)
    ...
    if(!heardIt &&
      ...
      else // Listen on all minor cycles if not long since last call for heat (so more calls are likely).
