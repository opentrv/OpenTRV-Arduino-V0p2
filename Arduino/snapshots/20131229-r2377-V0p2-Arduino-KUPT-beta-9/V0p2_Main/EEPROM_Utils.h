...
#define EE_START_SIMPLE_SCHEDULE_ON 12 // 1-byte encoded 'minutes after midnight' / 6 on-time, if any.
#define EE_START_SIMPLE_SCHEDULE_OFF 13 // 1-byte encoded 'minutes after midnight' / 6 off-time, if any.
...
#define EE_START_FHT8V_HC1 16 // (When controlling FHT8V rad valve): 1-byte value for house-code 1.
#define EE_START_FHT8V_HC2 17 // (When controlling FHT8V rad valve): 1-byte value for house-code 2.

// One byte BITWISE-INVERTED minimum number of minutes on boiler time; ~0 (erased/default) means NOT in hub/boiler mode.
// Bitwise-inverted so that erased/unset 0xff value leaves boiler mode disabled.
...
// One byte bias switch; ~0 (erased/default) means 'eco' energy-saving bias, else is comfort bias.
#define EE_START_ECO_BIAS 19 // If holds non-zero vaule then system has eco energy-saving bias.



// Housecode filter at centra hub.
