...
//  const bool rUIcu = recentUIControlUse();
  ...
  if(fastDigitalRead(BUTTON_MODE_L) == LOW)
    ...
    if(inWarmMode() || statusChange) // Generate flash(es) if in WARM mode or fiddling with UI other than Mode button.
      {
      ...
      if(statusChange ||
         ((forthTick || isCallingForHeat() || recentUIControlUse() || inBakeMode()) && (0 != readAmbientLight()) && !isRoomDark()))
        {
...
//  if(isLikelyOccupied()) { Serial.print(isLikelyRecentlyOccupied() ? 'O' : 'o'); } // Show room occupied and how recently.
  ...
  const uint8_t occupancy = occupancyConfidencePercent();
  if(occupancy > 0) { Serial.print(F(";o")); Serial.print(occupancy); }
