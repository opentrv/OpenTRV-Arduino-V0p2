...
uint8_t computeWARMTargetC(const uint8_t pot)
  {
#define TEMP_SCALE_LOW (BIASECO_WARM-1)
#define TEMP_SCALE_HIGH (BIASCOM_WARM+1)
#if defined(V0p2_REV)
#if 7 == V0p2_REV // Must match DORM1 scale 7 position scale 16|17|18|19|20|21|22 with frost/boost at extremes.
#if (16 != TEMP_SCALE_LOW) || (22 != TEMP_SCALE_HIGH)
#error Temperature scale must run from 16 to 22 inclusive for REV7 / DORM1 unit.
  ...
  return((pot / band) + TEMP_SCALE_LOW); // Intermediate (requires expensive run-time division).
  }
...
static const int maxRawOvershootC16 = 16 + refTempOffsetC16;

// Calculate reference temperature from measured temperature.
// The temperature regulation is in a 1C band.
...
void ModelledRadValve::computeTargetTemperature()
  ...
  if(!inWarmMode())
    ...
    if(inSmartMode())
      ...
      if(currentTempC < warmTarget) // Only bother with potentially-expensive computations if not already achieving WARM target!
        {
    ...
    // Set back target temperature a little if the room seems to have been vacant for a long time (TODO-107)
    ...
    if(longVacant ||
      ((AmbLight.isRoomDark() || notLikelyOccupiedSoon) && !isAnyScheduleOnWARMNow()))
      {
      // Use a default small non-annoying setback if in comfort mode
      //   or if the room is likely occupied now
      //   or if the room is lit or and hasn't been vacant for a very long time (TODO-107)
      ...
      // unless an even bigger 'full' setback if the room is unlikely to be unoccupied at this time of day or has been vacant for a very long time.
...
static bool glacialOn(ModelledRadValve const &mrv)
  ...
  // True if latest raw temperature nominally exceeds top of target range.
  const int rawTargetC16 = ((int)mrv.getTargetTempC())<<4;
  if(TemperatureC16.getRaw() > rawTargetC16 + refTempOffsetC16) { return(true); }
  // True if max recent raw temperature nominally significantly overshot top of target range.
  const int maxRecent = TemperatureC16.getRecentMaxRaw();
  if(maxRecent > rawTargetC16 + maxRawOvershootC16) { return(true); }
  // True if temperature range over recent history is wide (wider than full setback).
...
bool ModelledRadValve::computeRequiredTRVPercentOpen()
  ...
  if(refTempC < targetTempC) // (Well) under temp target: open valve.
    ...
    if(value < getMaxPercentageOpenAllowed())
      {
      ...
      if(inBakeMode()) { value = getMaxPercentageOpenAllowed(); valveTurnup(); return(true); }
#endif
      ...
      if(beGlacial) { ++value; }
        ...
        else { value = fnmin((uint8_t)(value + slewRate), getMaxPercentageOpenAllowed()); } // Capped at maximum.
        }

      // TRV setting has been changed (turned up).
      changed = true;
    ...
    {
    cancelBakeDebounced(); // Ensure BAKE mode cancelled immediately if over (BAKE) target.
    if(value != 0)
      {
...
//      const uint8_t lingerThreshold = DEFAULT_MIN_VALVE_PC_REALLY_OPEN - 1; // Should be below 'call for heat' threshold.
...
#endif
      // TODO-109: with comfort bias (or at hub because of self-heating temp errors) or if temperatures are jittery
      ...
      if(((!hasEcoBias()) || inHubMode() || TemperatureC16.isFilteringOn()) &&
         (value > constrain(((int)lingerThreshold) + TRV_SLEW_PC_PER_MIN_VFAST, TRV_SLEW_PC_PER_MIN_VFAST, getMaxPercentageOpenAllowed()))) { value -= TRV_SLEW_PC_PER_MIN_VFAST; }
      ...
      // TRV setting has been changed (turned down); always closes at least a little each time until fully closed.
      changed = true;
      // Defer reopening valve to avoid excessive hunting.
      valveTurndown();
      }
    }
    ...
    const uint8_t lsbits = (uint8_t) (refTempC16 & 0xf); // LSbits of temperature above base of proportional adjustment range.
...
//#if defined(VALVE_TURN_OFF_LINGER)
    ...
    const uint8_t minPCOpen = getMinValvePcReallyOpen();
    const uint8_t maxPCOpen = getMaxPercentageOpenAllowed();
    const uint8_t targetPO = constrain(targetPORaw, minPCOpen, maxPCOpen);
//#else
//    // Use as-is.
...
//#endif
    ...
    if(targetPO != value)
      ...
      // With a wider deadband also simply suppress any movement/noise on some/most minutes while close to temperature.
      const bool widenDeadband = AmbLight.isRoomDark() || Occupancy.longVacant();
      if(widenDeadband && (0 != (getMinutesSinceMidnightLT() & 3))) { return(false); } // Temporarily suppress movement entirely.
      // Also increase effective deadband if temperature resolution is lower than 1/16th, eg 8ths => 1+2*ulpStep minimum.
      const uint8_t realMinUlp = 1 + (TemperatureC16.isLowPrecision() ? 2*ulpStep : ulpStep); // Assume precision no coarser than 1/8C.
      const uint8_t minAbsSlew = fnmax(realMinUlp, (uint8_t)(widenDeadband ? min(25,max(TRV_MAX_SLEW_PC_PER_MIN,3*TRV_MIN_SLEW_PC)) : TRV_MIN_SLEW_PC));
      if(targetPO < value) // Currently open more than required, ie above target temperature.
        ...
        const bool beGlacial = (lsbits >= 14) || glacialOn(*this);
        if(beGlacial) { ++value; valveTurnup(); return(true); }

        // Slew open faster with comfort bias.
        const uint8_t maxSlew = (!hasEcoBias()) ? TRV_SLEW_PC_PER_MIN_FAST : TRV_MAX_SLEW_PC_PER_MIN;
        if(slew > maxSlew)
...
//  // Update the last-sample slot using the mean samples value.
//  eeprom_smart_update_byte((uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_TEMP_BY_HOUR) + hh), temp);
//  // If existing smoothed value unset or invalid, use new one as is, else fold in.
//  uint8_t *const phT = (uint8_t *)(EE_STATS_START_ADDR(EE_STATS_SET_TEMP_BY_HOUR_SMOOTHED) + hh);
...
// This will alter the content of the buffer for transmission,
...
//      maximise = true;
    ...
    ss1.put("tT|C", NominalRadValve.getTargetTempC());
//    ss1.put("tW|C", getWARMTargetC());
//    ss1.put("tF|C", getFROSTTargetC());
...
//    if(useExtraFHT8VTXSlots) { DEBUG_SERIAL_PRINTLN_FLASHSTRING("ES@1"); }
  ...
  switch(TIME_LSD) // With TWO_S_TICK_RTC_SUPPORT only even seconds are available.
    ...
    // At a hub, sample temperature as late as possible in the (and only in the 'quiet') minute, to reduce error from self-heating.
