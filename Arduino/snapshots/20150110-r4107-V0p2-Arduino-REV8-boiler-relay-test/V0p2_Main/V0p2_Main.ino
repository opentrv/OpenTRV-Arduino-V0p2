...
//// From: https://github.com/bblanchon/ArduinoJson if used...  MIT licence.
...
void serialPrintlnBuildVersion()
  ...
  serialPrintAndFlush(F("; code $Id: V0p2_Main.ino 4106 2015-01-10 16:03:32Z damonhd $ ")); // Expect SVN to substitute the Id keyword here with svn:keywords property set.
...
//  posPOST(1, F("Radio OK, checking buttons/sensors and xtal"));
  ...
  if(laterSCT == earlySCT)
    ...
    for(int i = 40; (--i >= 0) && (earlySCT == getSubCycleTime()); )
      {
      sleepLowPowerMs(203);
      captureEntropy1();
      sleepLowPowerMs(371);
      addEntropyToPool(noisyADCRead(), 1); // Churn the entropy pool...
      }
#endif
