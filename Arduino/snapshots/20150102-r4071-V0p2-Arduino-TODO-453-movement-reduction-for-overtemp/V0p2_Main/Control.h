...
// Typical valve percentage at which significant heat power is being provided.
#define DEFAULT_VALVE_PC_MODERATELY_OPEN 33
...
#define ANTISEEK_VALVE_RECLOSE_DELAY_M 3
...
#define ANTISEEK_VALVE_REOPEN_DELAY_M (ANTISEEK_VALVE_RECLOSE_DELAY_M*3)
...
// IF DEFINED allow a separate 'soft' setback of ~0.5C and gentle off.
//#define ALLOW_SOFT_SETBACK

#if defined(ALLOW_SOFT_SETBACK)
// If defined use the soft setback to deal with quick energy saving in darkness.
//#define QUICK_DARK_SETBACK_IS_SOFT
#endif
...
struct ModelledRadValveInputState
  ...
  bool inBakeMode;
#if defined(ALLOW_SOFT_SETBACK)
  // True if a soft temperature setback is required.
  // (TODO-451 - Have darkness only immediately trigger a 'soft setback')
  // Lets temperature drift downwards towards the bottom of the proportional-control range
  // with minimal valve movements and noise and in case (say) a room may be briefly unoccupied.
  bool softSetback;
#endif
...
struct ModelledRadValveState
  ...
  static const size_t filterLength = 8;
...
class OccupancyTracker : public SimpleTSUint8Sensor
  ...
  public:
    ...
    // 8h less than 2d may capture full office savings for the whole day of Sunday from EOD Friday.
