...
static void AssertIsTrue(bool x) { if(!(x)) { error(0); } }
static void AssertIsTrue(bool x, int err) { if(!(x)) { error(err); } }
...
// Test of FHT8V bitstream encoding.
// Aborts with panic() upon failure.
...
void testEntropyGathering()
  ...
  for(int i = MAX_IDENTICAL_BITS_SEQUENTIALLY; --i >= 0; )
    ...
    if(0 == i) { error(1); } // Generated too many identical values in a row. 
    }
  ...
  for(int i = MAX_IDENTICAL_BITS_SEQUENTIALLY; --i >= 0; )
    ...
    if(0 == i) { error(2); } // Generated too many identical values in a row. 
    }
  //DEBUG_SERIAL_PRINT_FLASHSTRING(" 1st=");
  ...
  for(int i = MAX_IDENTICAL_BITS_SEQUENTIALLY/8; --i >= 0; )
    ...
    if(0 == i) { error(3); } // Generated too many identical values in a row. 
    }
  //DEBUG_SERIAL_PRINT_FLASHSTRING(" 1st=");
...
// Tests generally flag an error with a call to panic() or error().
void unitTestLoop()
  {
  ...
  // Run tests, fastest or newest or most-fragile or most-interesting first...
