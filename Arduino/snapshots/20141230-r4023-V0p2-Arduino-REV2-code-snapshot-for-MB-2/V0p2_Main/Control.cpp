...
//DEBUG_SERIAL_PRINTLN_FLASHSTRING("well under temp");
    ...
    if(valvePCOpen < inputState.maxPCOpen)
      ...
      // Note that pseudo overshoots may be seen as the target temperature steps down.
      const bool beGlacial = glacialOn(inputState, retainedState);
...
//    // Note if there has been a big overshoot with valve open (ie heating on)
//    // so as to be more cautious opening valve the next time around...
...
//DEBUG_SERIAL_PRINTLN_FLASHSTRING("slightly over temp");
      ...
      if(slew > TRV_MAX_SLEW_PC_PER_MIN)
          { return(valvePCOpen - TRV_MAX_SLEW_PC_PER_MIN); } // Cap slew rate.
...
static const int MAX_TEMP_JUMP_C16 = 4; // 0.25C.
